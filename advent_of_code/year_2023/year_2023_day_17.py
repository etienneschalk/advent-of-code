...
# Literally copied from https://github.com/tbeu/AdventOfCode/blob/master/2023/day17/day17.cpp
...
# State is not only the node and its cost, but also the amount of steps in a given direction
...
def dijkstra(
    ...
    *,
    ...
    """Compute the shortest path from start to end in given heatmap, using min and max steps.
    ...
    min_step, optional
        Permitted minimal amount of consecutive steps in the same direction, inclusive, by default 1
    max_step, optional
        Permitted maximal amount of consecutive steps in the same direction, inclusive, by default 3

    Returns
    -------
        The shortest path from start to end in the heatmap.
    """
    # visited_states = set()
    visited_states: dict[int, State] = {}
    # Queue of State
    q = queue.PriorityQueue()  # P = ø
    for next_direction in NEIGHBOUR_MOVES.keys():
        # d[s_deb] = heat avec s_deb = start et heat = 0
        q.put(State(0, start, next_direction, 0))
    ...
    while not q.empty() and iter_count < max_iter:
        ...
        for next_direction, move in NEIGHBOUR_MOVES.items():
            ...
            if next_state_hash in visited_states:
                # I cannot rely on the hash for set usage, so I use directly manually my own hash
                # in a dict. The idea is that the heat does not participate in the hash.
                current_stored_state = visited_states[next_state_hash]
                if next_heat < current_stored_state.heat:
